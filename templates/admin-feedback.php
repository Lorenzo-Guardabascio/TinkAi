...
if (isset($_GET['action']) && $_GET['action'] === 'export' && current_user_can('manage_options')) {
    ...
    fputcsv($output, ['ID', 'User ID', 'Rating', 'Variant', 'What Worked', 'What Failed', 'Suggestion', 'Question', 'AI Response', 'Created At']);
...
$variant_filter = isset($_GET['variant']) ? sanitize_text_field($_GET['variant']) : '';
$variant_where = $variant_filter ? $wpdb->prepare("AND f.variant_name = %s", $variant_filter) : "";

$total = $wpdb->get_var("SELECT COUNT(*) FROM $table f WHERE 1=1 $variant_where");
...
$variants = $wpdb->get_col("SELECT DISTINCT variant_name FROM $table WHERE variant_name IS NOT NULL AND variant_name != '' ORDER BY variant_name");
?>
...
<div class="wrap">
    ...
    <div class="tablenav top">
        <div class="alignleft actions">
            <select id="variant-filter" onchange="window.location.href='?page=tinkai-feedback&variant='+this.value">
                <option value="">All Variants</option>
                <?php foreach ($variants as $v): ?>
                    <option value="<?php echo esc_attr($v); ?>" <?php selected($variant_filter, $v); ?>>
                        <?php echo esc_html($v ?: 'default'); ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <a href="?page=tinkai-feedback&action=export<?php echo $variant_filter ? '&variant='.urlencode($variant_filter) : ''; ?>" class="button">
                📥 Export to CSV
            </a>
        </div>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="variant-badge">
                                <?php echo esc_html($item['variant_name'] ?: 'default'); ?>
                            </span>
                        </td>
